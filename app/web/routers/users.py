...
async def profile(request: Request, user_id: int,
    ...
    rooms = await RoomRepo().get_all_users_of_room(user_id)
...
async def users(request: Request, page: int = 1, limit: int = 10,
    ...
    users, total_users = await PaginatorRepo().paginate(User, page, limit)
    context = {
        'request': request,
        'current_user': current_user,
        'users': users,
...
async def activate(request: Request, user_id: int,
    ...
    """
    user = await UserRepo().get_user_or_none(user=user_id)
    ...
    if user.is_active:
        ...
        await UserRepo().enable_user(user_id)
