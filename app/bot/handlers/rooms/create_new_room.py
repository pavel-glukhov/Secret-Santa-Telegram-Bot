...
async def create_room(callback: types.CallbackQuery,
    ...
    if count_user_rooms >= load_config().room.user_rooms_count:
        ...
        message_text = app_text_msg.messages.rooms_menu.create_new_room.limit
...
async def process_budget(message: types.Message, state: FSMContext, app_text_msg: TranslationMainSchema):
...
async def process_wishes(message: types.Message, state: FSMContext, session: Session, app_text_msg: TranslationMainSchema):
