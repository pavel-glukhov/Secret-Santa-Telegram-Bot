## Secret Santa Telegram Bot
[![en](https://img.shields.io/badge/lang-en-green.svg)](https://github.com/pavel-glukhov/Secret-Santa-Telegram-Bot/blob/main/README.md)
[![ru](https://img.shields.io/badge/lang-ru-yelow.svg)](https://github.com/pavel-glukhov/Secret-Santa-Telegram-Bot/blob/main/README.ru.md)

## Описание
Бот для организации игры Secret Santa в Telegram. Позволяет участникам легко проводить жеребьевку, обмениваться пожеланиями и получать результаты. Также поддерживается анонимное общение между участником и его Секретным Санта.

Поддерживаемые языки:
- RU
- ENG
- KAZ
- UK

### Текущая версия: 0.4.2

---
## Changelog

**v 0.4.3**
- Обновлены диалоги
- Добавлена кнопка 'пригласить друга' в меню комнаты
- Исправлены некоторые мелкие ошибки.

**v 0.4.2**
* Логика запуска приложения переработана и разделена на runtime.
  - Добавлены независимые режимы запуска:
    - Polling — для локальной разработки и тестирования.
    - Webhook — для продакшена с HTTPS.

**v 0.4.1**
* Возможность присоединения к комнате по URL.
* Обновлены диалоги.
* Исправлена логика приложения.

**v 0.4.0**
* Все ORM-запросы переписаны на async.
* Исправлены ошибки интерфейса.

**v 0.3.0**
* Добавлен интерактивный календарь для планирования жеребьевки.
* Реализована случайная отправка результатов в указанные периоды.
* Исправлены различные ошибки.

**v 0.2.2**
* Исправлены ошибки отправки сообщений.
* Оптимизированы SQL-запросы.

**v 0.2.1**
* Исправлен баг с получением пожеланий игроков.
* Обновлены языковые словари.

**v 0.2.0**
* Миграция с Aiogram 2 на Aiogram 3.
* SQLAlchemy заменил Tortoise ORM.
* Добавлена поддержка нескольких языков.

**v 0.1.0**
* Beta релиз.

---

Диаграмма: https://miro.com/app/board/uXjVNxWmMtE=/?share_link_id=603886678614

## Стек технологий

- Aiogram 3
- FastAPI
- SQLAlchemy 2
- Alembic
- PostgreSQL
- Redis

## Возможности

### 1. Создание персональных комнат

- Любой пользователь может создать комнату для неограниченного количества участников.
    - При создании необходимо указать:
        - **Название комнаты**
        - **Бюджет**
        - **Пожелания к подарку**

```
Переменная окружения USER_ROOMS_LIMIT задаёт лимит комнат, которыми может управлять пользователь.
```

```
При создании комнаты генерируется случайный уникальный номер, его длина задаётся параметром ROOM_NUMBER_LENGTH.
```

### 2. Вход в существующую комнату по ID

### 3. Управление профилем

- Бот позволяет вводить и изменять:
    - **Имя и фамилию**
    - **Домашний адрес**
    - **Номер телефона**
    - **Часовой пояс**
    - **Язык интерфейса**
    - **Полное удаление данных**

```
Адрес и номер телефона шифруются в базе данных с использованием алгоритма Fernet.
```

### 4. Управление комнатой

- Меню для обычных пользователей:
    - **Покинуть комнату**
    - **Изменить пожелания**
- Меню для владельца комнаты:
    - **Начать игру** — настройка времени распределения
        - **Настройка часового пояса**
    - **Изменить пожелания**
    - **Список участников**
    - **Пригласить друга**
    - **Настройки**
        - **Удалить комнату**
        - **Изменить название комнаты**
        - **Сменить владельца**
        - **Изменить бюджет**

```
Администратор комнаты не может покинуть её, пока не передаст управление другому пользователю. Комната может быть удалена только полностью.
```

```
Если во время жеребьёвки недостаточно игроков, комнату можно активировать повторно.
```

### 5. Общение

После распределения ролей становятся доступны анонимные каналы общения:

- **Отправить сообщение Тайному Санте**
- **Отправить сообщение получателю подарка**

---

## Запуск бота

### Подготовка
Создайте `.env` согласно примеру `.env.example`.

## Environment Variables
| Category | Variable | Description | Default Value | Notes |
|--------|----------|-------------|---------------|---|
| **Security** | `ENCRYPT_SECRET_KEY` | Base64-кодированный 32-байтовый секретный ключ для шифрования данных | ❌ *нет* | Обязателен. Генерация: `python manage.py generate_key` |
| **Telegram Bot** | `TELEGRAM_TOKEN` | Токен Telegram-бота от @BotFather | ❌ *нет* | Обязателен |
|  | `TELEGRAM_LOGIN` | Username бота без `@` | ❌ *нет* | Пример: `my_bot_name` |
|  | `SUPPORT_ACCOUNT` | Контакт поддержки (Telegram) | ❌ *нет* | Отображается на странице About |
| **Room Configuration** | `ROOM_NUMBER_LENGTH` | Длина генерируемого ID комнаты | `6` | Допустимо: `5–8` |
|  | `USER_ROOMS_LIMIT` | Максимальное число комнат на пользователя | `3` | |
| **Database (PostgreSQL)** | `DATABASE_NAME` | Название базы данных | ❌ *нет* | |
|  | `DATABASE_USER` | Пользователь БД | ❌ *нет* ||
|  | `DATABASE_PASSWORD` | Пароль пользователя БД | ❌ *нет* | ⚠️ хранить в секрете |
|  | `DATABASE_HOST` | Хост БД | `db` | `db` — Docker, `localhost` — локально |
|  | `DATABASE_PORT` | Порт PostgreSQL | `5432` ||
|  | `POOL_SIZE` | Количество постоянных соединений в пуле | `5` | |
|  | `MAX_OVERFLOW` | Макс. временных соединений сверх пула | `10` | |
| **Redis** | `REDIS_HOST` | Хост Redis | `redis` | `redis` — Docker, `localhost` — локально |
|  | `REDIS_PORT` | Порт Redis | `6379` ||
|  | `REDIS_DB` | Номер базы Redis | `0` | |
|  | `REDIS_PASSWORD` | Пароль Redis | *(пусто)* | Если не используется |
| **Webhook & SSL (Certbot)** | `DOMAIN_NAME` | Домен для webhook (без http/https) | ❌ *нет* | Обязателен для webhook |
|  | `EMAIL` | Email для Certbot | ❌ *нет* | Обязателен для webhook |
| **System** | `SERVER_TIMEZONE` | Таймзона сервера | `Asia/Almaty` ||


### Режимы запуска

Бот поддерживает два независимых режима работы. Выбор режима зависит от нагрузки, инфраструктуры и наличия публичного HTTPS-домена.

---

## 🔁 Polling Mode (Long Polling)

**Описание**
- Используется long polling Telegram API
- **Не требует** публичного HTTPS
- Самый простой вариант запуска
- Подходит для:
  - разработки
  - ботов с низкой нагрузкой
  - серверов без домена

**Запуск локально**
```bash
python -m app.runtimes.polling
```

**Запуск через Docker**
```bash
docker-compose -f docker-compose_long_polling.yaml up
```

**Примечания**
- Telegram сам опрашивает бота
- Чуть большая задержка по сравнению с webhook
- Допускается только **один экземпляр polling**

---

## 🌐 Webhook Mode

**Описание**
- Используются Telegram webhooks
- Требуется **публичный HTTPS endpoint**
- Рекомендуется для продакшена
- Меньшая задержка и лучшая масштабируемость

---

## Локальное тестирование Webhook

Для локального тестирования webhook можно использовать туннель (например, `ngrok`).

```bash
uvicorn app.runtimes.webhook:create_app --reload
```

После этого пробросьте порт наружу и зарегистрируйте webhook на выданный HTTPS-URL.

---

## Продакшен (Webhook)

### Требования
- Публичный домен
- Открытые порты **80** и **443**
- Валидный HTTPS-сертификат

### Запуск
```bash
uvicorn app.runtimes.webhook:create_app
```

### Регистрация Webhook

Webhook можно зарегистрировать несколькими способами:

**Автоматически**  
Рекомендуемый вариант — регистрация при старте приложения.

**Через management-команду**
```bash
python manage.py register_webhook
```

**Вручную через Telegram API**
```text
https://api.telegram.org/bot{telegram_token}/setWebhook?url=https://{domain_name}/bot/
```

---

## HTTPS и Caddy

В продакшене webhook работает **через Caddy**.

- Reverse proxy
- Автоматическое получение и обновление SSL (Let’s Encrypt)
- Проксирование HTTPS-запросов в backend

Поток трафика:

```
Telegram → HTTPS → Caddy → backend
```

---

## 🐳 Docker-режимы

| Режим           | Команда |
|-----------------|---------|
| Polling         | `docker-compose -f docker-compose_long_polling.yaml up` |
| Webhook (Caddy) | `docker-compose up` |
| Webhook (Nginx) | `chmod +x nginx-entrypoint.sh`<br/>`docker-compose up -f docker-compose_nginx.yaml up`|
---

## Установка Docker

Перед запуском необходимо установить Docker.

### Установка Docker Engine

Используйте официальную документацию для вашей ОС:

👉 https://docs.docker.com/engine/install/

После установки проверьте:
```bash
docker --version
docker compose version
```

Убедитесь, что Docker daemon запущен.

---

## Резервное копирование

### Бэкап базы данных

```bash
docker exec -t <container_name> pg_dump -U <username> <database_name> > <file_name>.sql
```

### Скрипты бэкапа

Также доступны скрипты в каталоге:
```
deploy/backup
```

Рекомендуется:
- запускать бэкапы по cron
- хранить копии вне VPS

---

## Управление правами доступа

### Назначить суперпользователя

```bash
python manage.py set_superuser <telegram_user_id>
```

### Удалить суперпользователя

```bash
python manage.py remove_superuser <telegram_user_id>
```
