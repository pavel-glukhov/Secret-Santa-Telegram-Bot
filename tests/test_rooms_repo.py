...
async def test_create_room(room_repo, session):
    user = User(user_id=2000001, username="room_owner931")
...
async def test_add_member(room_repo, session):
    user = User(user_id=2000002, username="member")
    room = Room(name="Room_add_tst", budget="200", owner_id=2000001, number=100103)
    add_and_commit(session, user, room)


    room = session.query(Room).filter_by(name='Room_add_tst').first()
    result = await room_repo.add_member(user_id=2000002, room_number=room.number)
...
async def test_remove_member(room_repo, session):
    user = User(user_id=2000003, username="member2")
    room = Room(name="Room3", number=98765, budget="300", owner_id=2000003)
    room.members.append(user)
    add_and_commit(session, user, room)

    await room_repo.remove_member(user_id=2000003, room_number=98765)
    assert user not in room.members

@pytest.mark.asyncio

async def test_is_member(room_repo, session):
    user = User(user_id=2000004, username="testuser_323")

    room = Room(name="Room4", number=13579, budget="400$", owner_id=2000004)
    room.members.append(user)
    add_and_commit(session, user, room)

    assert await room_repo.is_member(user_id=2000004, room_number=13579) is True
    assert await room_repo.is_member(user_id=2000099, room_number=13579) is False


@pytest.mark.asyncio
async def test_is_room_owner(room_repo, session):
    room = Room(name="Room5", number=24680, budget="500$", owner_id=2000001)
    add_and_commit(session,  room)

    assert await room_repo.is_owner(user_id=2000001, room_number=24680) is True
    assert await room_repo.is_owner(user_id=2000002, room_number=24680) is False

@pytest.mark.asyncio
async def test_delete_room(room_repo, session):
    room = Room(name="Room6", number=111222, budget="600", owner_id=2000001)
    add_and_commit(session,  room)
