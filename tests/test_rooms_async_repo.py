...
base_data = {
    ...
    "users": {
        "user1": {"user_id": 4000001,
                  "username": "test_user_1",
                  "language": "en",
                  "first_name": "first_name_test_user 1",
                  "last_name": "last_name_test_user 1",
                  "is_active": True,
                  },
        "user2": {"user_id": 4000002,
                  "username": "test_user_2",
                  "language": "en",
                  "first_name": "first_name_test_user 2",
                  "last_name": "last_name_test_user 2",
                  "is_active": True,
                  }
    }
}
...
async def test_create_room(room_repo, session):
    ...
    await session.refresh(room, ["members"])
...
async def test_update_room(room_repo, session):
    ...
    result = await session.execute(select(Room).filter_by(name='room_update_test'))
    room = result.scalars().first()

    await room_repo.update(room_number=room.number,
                           name="updated_name", budget="150$")
    result = await session.execute(select(Room).filter_by(name="updated_name"))
    updated_room = result.scalars().first()
...
async def test_add_member(room_repo, session):
    ...
    result = await session.execute(select(Room).filter_by(name='room_add_tst'))
    room = result.scalars().first()
...
async def test_remove_member(room_repo, session):
    ...
    await add_and_commit(session, new_member, room, user)
...
async def test_is_room_owner(room_repo, session):
    ...
    await add_and_commit(session, is_no_member, room, user)
...
async def test_delete_room(room_repo, session):
    ...
    result = await session.execute(select(Room).filter_by(number=111222))
