...
async def test_update_user(user_repo, session, user):
    ...
    updated_user = await session.get(User, user.user_id)
...
async def test_disable_enable_user(user_repo, session, user):
    ...
    disabled_user = await session.get(User, user.user_id)
    assert disabled_user.is_active is False

    await user_repo.enable_user(user.user_id)
    enabled_user = await session.get(User, user.user_id)
...
async def test_list_rooms_where_owner(user_repo, session):
    ...
    await add_and_commit(session, room1, room2, user)
...
async def test_delete_user(user_repo, session, user):
    ...
    deleted_user = await session.get(User, user.user_id)
